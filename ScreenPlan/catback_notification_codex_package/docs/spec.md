...
This package now also includes notification UI references.

## 16.1 Purpose
The notification page informs customers about important updates related to:
- cashback
- orders
- wallet
- promotions
...
## 16.2 Main layout
The approved notification UI includes:
1. hero header
2. notification bell and settings/filter icon
3. unread badge count
4. chip filters
5. grouped notification list
6. bottom navigation

## 16.3 Header
Header text:
- title: `Thông báo`
- subtitle: `Cập nhật hoàn tiền, đơn hàng và ví của bạn`

Header actions:
- notification bell with unread badge
- filter/settings icon

When unread count = 0, the badge may be hidden.
...
- active chip uses Catback teal filled state
- inactive chips are white with subtle border
...
## 16.7 Suggested customer DTO
```csharp
public class CustomerNotificationListItemDto
{
    public Guid Id { get; set; } // route only, never render as text
...
## 16.9 Suggested Razor structure
```text
Pages/
└── Notifications/
    ├── Index.cshtml
    ├── Index.cshtml.cs
    ├── _NotificationHeader.cshtml
    ├── _NotificationFilterChips.cshtml
    ├── _NotificationSection.cshtml
    ├── _NotificationCard.cshtml
    └── _UnreadBadge.cshtml
```

## 16.10 Visual references
Use:
- `references/notification/notification-full.png`
- `references/notification/notification-header-tabs.png`
- `references/notification/notification-today-list.png`
- `references/notification/notification-previous-list.png`
- `references/notification/notification-bottom-nav.png`
- `references/notification/notification-single-card-example.png`

Use blueprints:
- `blueprints/notification-mobile.svg`
- `blueprints/notification-header-tabs.svg`
