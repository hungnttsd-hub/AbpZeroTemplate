...
# 17. Shared modal system

This package now also includes the Catback shared modal system.

## 17.1 Purpose
Use a reusable Catback modal component instead of the browser default `alert()` / `confirm()` dialogs.

This shared modal should be reused across the entire project for:
- confirmations
- destructive actions
- success feedback
...
## 17.2 Main variants
The design package includes 3 core variants:

1. **Warning / destructive confirmation**
   - Example: delete notification
   - Primary destructive action: `Xóa`
   - Secondary action: `Hủy`

2. **Success**
   - Example: delete success, save success
   - Single action: `Đóng`

3. **Info**
   - Example: notice, short instruction
   - Single action: `Đã hiểu`

## 17.3 Standard structure
Every modal should support:
- overlay backdrop
- dialog container
- optional close icon
- top icon / status icon
- title
- description
- footer actions
...
- Overlay darkens the background to focus attention.
- Destructive primary action should use the warning/destructive color.
- Secondary action should be outline or neutral.
- The dialog radius and spacing should follow the approved Catback style.

## 17.5 Suggested DTO / ViewModel
```csharp
public class AppModalViewModel
{
    public string Title { get; set; } = default!;
    public string? Message { get; set; }
    public string Variant { get; set; } = "info"; // warning | success | info
    public string PrimaryButtonText { get; set; } = "Đóng";
    public string? SecondaryButtonText { get; set; }
    public bool ShowCloseButton { get; set; } = true;
}
```

## 17.6 Suggested Razor structure
```text
Pages/
└── Shared/
    ├── _AppModal.cshtml
...
showInfoModal({
  title: "Thông tin",
  message: "Đây là nội dung thông tin hoặc hướng dẫn đến người dùng."
});
```

## 17.8 Interaction rules
- The Escape key should close non-blocking modals when appropriate.
- For destructive confirmations, consider disabling click-outside-to-close.
- Focus should move into the modal when opened.
- Return focus to the previously focused element after closing.
- Keep button order consistent across the project.

## 17.9 Visual references
Use:
- `references/modal/modal-guideline-full.png`
- `references/modal/modal-main-example.png`
- `references/modal/modal-component-notes.png`
- `references/modal/modal-variants-row.png`
- `references/modal/modal-usage-guidelines.png`
- `references/modal/modal-header-title.png`

Use blueprints:
- `blueprints/modal-confirmation.svg`
- `blueprints/modal-variants.svg`
