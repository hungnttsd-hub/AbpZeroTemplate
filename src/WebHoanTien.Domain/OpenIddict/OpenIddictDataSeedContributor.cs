...
namespace WebHoanTien.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("WebHoanTien") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "WebHoanTien", DisplayName = "WebHoanTien API", Resources = { "WebHoanTien" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["WebHoanTien_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["WebHoanTien_Web:RootUrl"]!.EnsureEndsWith('/');

            /* WebHoanTien_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["WebHoanTien_Web:ClientSecret"]
                    ?? throw new AbpException("OpenIddict Web client secret must be configured outside source control."),
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["WebHoanTien_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["WebHoanTien_Swagger:RootUrl"]?.TrimEnd('/');
