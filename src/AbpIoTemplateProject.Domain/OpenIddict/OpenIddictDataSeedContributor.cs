...
namespace AbpIoTemplateProject.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("AbpIoTemplateProject") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "AbpIoTemplateProject", DisplayName = "AbpIoTemplateProject API", Resources = { "AbpIoTemplateProject" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        var commonScopes = new List<string> {
            ...
            "AbpIoTemplateProject"
        };

        var configurationSection = _configuration.GetSection("OpenIddict:Applications");

        //Web Client
        var webClientId = configurationSection["AbpIoTemplateProject_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["AbpIoTemplateProject_Web:RootUrl"]!.EnsureEndsWith('/');

            /* AbpIoTemplateProject_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["AbpIoTemplateProject_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["AbpIoTemplateProject_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["AbpIoTemplateProject_Swagger:RootUrl"]?.TrimEnd('/');
