...
export class WordCompletionController {
  ...
  start(config: WordBuilderConfig, slots: LetterSlot[], action: CompletionAction) {
    ...
    this.emit('WORD_COMPLETED'); this.emit('COMPLETION_ACTION_STARTED');
    ...
    action.play(() => { if (this.disposed) return; this.emit('COMPLETION_ACTION_FINISHED'); actionDone = true; ready(); });
    const units = config.unit === 'word' ? config.targetWord.split(' ') : [...config.targetWord];
    void this.ctx.audio.speakSequence([...units, config.targetWord, config.example], index => {
      if (this.disposed) return;
      if (index < slots.length) { slots[index].glow(); this.effects.bounce(slots[index].view); }
      else slots.forEach(slot => slot.glow());
    }).finally(() => { voiceDone = true; ready(); });
  }
  private showContinue() {
    const s = this.ctx.scene;
    const background = s.add.rectangle(0, 0, 300, 82, 0x799d6a).setStrokeStyle(4, 0xd9e4bd);
    const text = s.add.text(0, 0, '★  Nhận ngôi sao  →', { fontFamily: 'Nunito, Arial', fontSize: '26px', color: '#ffffff', fontStyle: 'bold' }).setOrigin(.5);
