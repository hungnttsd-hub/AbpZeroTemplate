...
import { ArrowLeft, ArrowRight, Bell, BookOpen, Check, ChevronLeft, ChevronRight, Compass, Play, Search, Sparkles, Star, Volume2, VolumeX } from 'lucide-react';
...
export default function GoldenBellPage({ child, repository, levels, progress, muted, onMute, onBack }: { child: Child; repository: GameRepository; levels: LevelDefinition[]; progress: Progress[]; muted: boolean; onMute: () => void; onBack: () => void }) {
  ...
  return <main className="gb-lobby" style={{ '--gb-backdrop': `url("${artUrl('golden-bell-island-v1.png')}")` } as React.CSSProperties}>
    ...
    <section className="gb-stats" aria-label="Hành trình Chuông Sao"><div><BookOpen/><strong>{bank?.count ?? '…'}</strong><span>Câu hỏi sẵn sàng</span></div><div><Check/><strong>{completed.size}</strong><span>Câu đã khám phá</span></div><div><Bell/><strong>{store?.history.tokens ?? 0}</strong><span>Bell Tokens</span></div><div><Star/><strong>{store?.history.sessions ?? 0}</strong><span>Lần đánh thức chuông</span></div></section>
